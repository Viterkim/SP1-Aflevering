...
public class UrlRetriever extends Thread {
    ...
    public static void main(String[] args) 
    {
        try
        {
            UrlRetriever ret = new UrlRetriever(new URL("http://www.viter.dk"));
            ret.start();
            ret.join();
            Optional<byte[]> endData = ret.getData();
     ...
     */
    public Optional<byte[]> getData() throws IllegalStateException, IOException 
    {
        Optional<byte[]> arr = Optional.ofNullable(byteArr);
        return arr;
    }
